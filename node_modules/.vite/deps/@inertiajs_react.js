import { i as __toESM, n as __commonJSMin, r as __exportAll, t as require_react } from "./react-CbQW63mU.js";
...
var hasOwnProperty$15 = objectProto$4.hasOwnProperty;
/**
* Used to resolve the
...
/** Used to check objects for own properties. */
var hasOwnProperty$14 = objectProto$3.hasOwnProperty;
...
/** Used to check objects for own properties. */
var hasOwnProperty$12 = objectProto$1.hasOwnProperty;
/** Built-in value references. */
...
var Scroll = class {
	...
	static reset() {
		if (!(typeof window !== "undefined" ? window.location.hash : null)) window.scrollTo(0, 0);
...
var CurrentPage = class {
	...
	set(page2, { replace = false, preserveScroll = false, preserveState = false, viewTransition = false } = {}) {
		if (Object.keys(page2.deferredProps || {}).length) this.pendingDeferredProps = {
	...
	swap({ component, page: page2, preserveState, viewTransition }) {
		...
		if (!viewTransition || !document?.startViewTransition) return doSwap();
...
var History = class {
	...
	restore(key) {
		if (!isServer) return this.current[this.rememberedState]?.[key] !== void 0 ? this.current[this.rememberedState]?.[key] : this.initialState?.[this.rememberedState]?.[key];
	}
	...
	decrypt(page2 = null) {
		if (isServer) return Promise.resolve(page2 ?? page.get());
	...
	*/
	...
	static handleDefault() {
		...
		page.set(page.get(), {
			...
			if (Object.keys(flash).length > 0) fireFlashEvent(flash);
		});
	}
};
var Poll = class {
...
var RequestStream = class {
	...
	cancel({ cancelled = false, interrupted = false } = {}, force) {
		if (!this.shouldCancel(force)) return;
...
var useInfiniteScrollData = (options) => {
	...
	const removeEventListener = router.on("success", (event) => {
		if (state.component === event.detail.page.component && getScrollPropFromCurrentPage().reset) resetState();
	});
...
function resetFormFields(formElement, defaults, fieldNames) {
	...
	if (hasChanged && resetEntireForm) formElement.dispatchEvent(new Event("reset", { bubbles: true }));
}
	...
	const resolveConfig = (globalConfig, instanceConfig, data = {}) => {
		...
		return {
			...
			onValidationError: (response, axiosError) => {
				[...setValidated([...validated, ...only]), ...setErrors(merge$1(omit({ ...errors }, only), response.data.errors))].forEach((listener) => listener());
			...
			onPrecognitionSuccess: (response) => {
				[...setValidated([...validated, ...only]), ...setErrors(omit({ ...errors }, only))].forEach((listener) => listener());
...
function usePage() {
	const page = typeof import_react.use === "function" ? import_react.use(PageContext_default) : import_react.useContext(PageContext_default);
...
var Deferred = ({ children, data, fallback }) => {
	...
	if (loaded) return typeof children === "function" ? children() : children;
...
function useForm(...args) {
	...
	const [data, setData] = rememberKey ? useRemember(defaults, `${rememberKey}:data`) : (0, import_react.useState)(defaults);
	...
	const withAllErrors = (0, import_react.useRef)(false);
	(0, import_react.useEffect)(() => {
	...
	const validate = (field, config2) => {
		...
		if (field === void 0) validatorRef.current.validate(config2);
		else {
...
var Form = (0, import_react.forwardRef)(({ action = "", method = "get", headers = {}, queryStringArrayFormat = "brackets", errorBag = null, showProgress = true, transform = (data) => data, options = {}, onStart = noop, onProgress = noop, onFinish = noop, onBefore = noop, onCancel = noop, onSuccess = noop, onError = noop, onCancelToken = noop, onSubmitComplete = noop, disableWhileProcessing = false, resetOnError = false, resetOnSuccess = false, setDefaultsOnSuccess = false, invalidateCacheTags = [], validateFiles = false, validationTimeout = 1500, withAllErrors = false, children, ...props }, ref) => {
	...
	const updateDirtyState = (event) => deferStateUpdate(() => setIsDirty(event.type === "reset" ? false : !isEqual(getData(), formDataToObject(defaultData.current))));
	...
	const exposed = () => ({
		...
		validate: (field, config2) => form.validate(...UseFormUtils.mergeHeadersForValidation(field, config2, headers)),
		touch: form.touch,
	...
	return (0, import_react.createElement)("form", {
		...props,
		ref: formElement,
...
var WhenVisible = ({ children, data, params, buffer, as, always, fallback }) => {
	...
	const getReloadParams = (0, import_react.useCallback)(() => {
...
//#endregion
export { App, Deferred_default as Deferred, Form_default as Form, Head_default as Head, InfiniteScroll_default as InfiniteScroll, Link_default as Link, WhenVisible_default as WhenVisible, config, createInertiaApp, progress, router3 as router, useForm, usePage, usePoll, usePrefetch, useRemember };
