...
use Modules\SAS\Notifications\SASNotification;
...
class SASNotificationService
{
    ...
    public function sendInsuranceExpirationWarnings(): int
    {
        ...
        $insurances30 = InsuranceRecord::where('status', 'Approved')
            ->whereDate('expiration_date', $thirtyDaysOut->format('Y-m-d'))
            ->with('student')
            ->get();
        ...
        $insurances7 = InsuranceRecord::where('status', 'Approved')
            ->whereDate('expiration_date', $sevenDaysOut->format('Y-m-d'))
            ->with('student')
            ->get();
