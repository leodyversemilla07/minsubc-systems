...
export default function SystemSettings({ settings, filters, types }: SystemSettingsProps) {
    const [searchTerm, setSearchTerm] = useState(filters.search || '');
    const [selectedType, setSelectedType] = useState(filters.type === '' ? 'all' : filters.type);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex-1 space-y-8 p-6 md:p-8">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>System Settings ({settings.total})</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                    ...
                                    <TableRow key={setting.id}>
                                        ...
                                        <TableCell>
                                            <div className="text-sm">
                                                <div>{new Date(setting.updated_at).toLocaleDateString()}</div>
                                                {setting.user && (
                                                    <div className="text-muted-foreground">
                                                        by {setting.user.first_name} {setting.user.last_name}
                                                    </div>
                                                )}
                                            </div>
                                        </TableCell>
                ...
                <Dialog open={!!editingSetting} onOpenChange={() => setEditingSetting(null)}>
                    <DialogContent className="sm:max-w-[500px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Update the value for <code className="bg-muted px-1 rounded">{editingSetting?.setting_key}</code>
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                {editingSetting?.value && editingSetting.value.length > 100 ? (
                                    <Textarea
                                        id="value"
                        ...
                        <DialogFooter>
                            ...
                            <Button onClick={handleUpdateSetting} disabled={form.processing}>
