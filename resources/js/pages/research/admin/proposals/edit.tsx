...
export default function Edit({ proposal, researchTypes }: { proposal: any; researchTypes: ResearchType[] }) {
    const { data, setData, put, processing, errors } = useForm({
        title: proposal.title,
        abstract: proposal.abstract ?? '',
        research_type_id: String(proposal.research_type_id ?? ''),
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('research.admin.proposals.update', proposal.id));
    };
                    ...
                    <CardContent>
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <Textarea id="abstract" value={data.abstract} onChange={(e) => setData('abstract', e.target.value)} rows={6} />
