...
export default function Requirements({ recipient, requirements, stats }: Props) {
    ...
    const isOverdue = (requirement: ScholarshipRequirement) => {
        return requirement.deadline && new Date(requirement.deadline) < new Date() && !requirement.is_submitted;
    };

    return (
        <AppLayout>
            <Head title={`Requirements - ${recipient.scholarship?.scholarship_name}`} />
            ...
            <div className="container mx-auto py-8 px-4">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-muted-foreground">
                        {recipient.scholarship?.scholarship_name} - {recipient.academic_year} ({recipient.semester} Semester)
                    </p>
                </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-sm font-medium">{stats.completion_percentage}%</span>
                                </div>
                ...
                <div className="space-y-4">
                    {requirements.length === 0 ? (
                        <Alert>
                            ...
                            <AlertDescription>
                                No requirements have been set for this scholarship yet.
                            </AlertDescription>
                        </Alert>
                    ) : (
                        requirements.map((requirement) => (
                            <Card 
                                key={requirement.id} 
                                className={isOverdue(requirement) ? 'border-red-500' : ''}
                            >
                                <CardHeader>
                                    ...
                                    {requirement.is_submitted ? (
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-2 text-sm text-green-600">
                                                ...
                                                Submitted on {formatDate(requirement.submission_date!)}
                                            </div>
                                            ...
                                            {requirement.file_path && (
                                                <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
                                                    ...
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                                        onClick={() => window.open(requirement.file_path, '_blank')}
                                                    >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-2">
                                                <Input
                                                    ...
                                                    disabled={processing && uploadingId === requirement.id}
                                                    className="flex-1"
                                                />
                                                <Button
                                                    type="button"
                                                ...
                                                >
                                                    ...
                                                    {uploadingId === requirement.id && processing ? 'Uploading...' : 'Upload'}
                                                </Button>
                                            </div>
                                            {errors.file && (
                ...
                <Card className="mt-6">
                    ...
                    <CardContent>
                        <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
                            <li>Upload clear and legible copies of all required documents</li>
                            <li>Accepted file formats: PDF, JPG, PNG (Maximum 10MB per file)</li>
                            <li>Ensure all documents are complete and properly signed where required</li>
                            <li>Submit documents before the deadline to avoid scholarship suspension</li>
                            <li>Contact the SAS Office if you have questions about any requirement</li>
                        </ul>
                    </CardContent>
                </Card>
            </div>
