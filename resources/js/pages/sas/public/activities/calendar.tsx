...
export default function ActivitiesCalendar({ activities, startDate, endDate }: Props) {
            ...
            <SASLayout>
                ...
                <div className="flex min-h-screen items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-gray-600">Please wait while we load the calendar.</p>
                    </div>
                </div>
            </SASLayout>
        );
    ...
    for (let i = 0; i < startingDayOfWeek; i++) {
        calendarDays.push(<div key={`empty-${i}`} className="min-h-24 border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800"></div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        calendarDays.push(
            ...
            >
                <div className={`mb-1 text-sm font-semibold ${isToday ? 'text-blue-700 dark:text-blue-400' : 'text-gray-900 dark:text-white'}`}>
                    {day}
                </div>
                    ...
                    {dayActivities.slice(0, 3).map((activity) => (
                        ...
                        >
                            <div className="rounded bg-blue-100 px-2 py-1 text-xs font-medium text-blue-900 transition-colors hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-100 dark:hover:bg-blue-800">
                                <div className="truncate">{activity.activity_title}</div>
                                {!activity.all_day && (
                                            ...
                                            <CardContent className="flex items-start gap-4 p-4">
                                                <div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900">
                                                    <div className="text-xs font-medium text-blue-700 dark:text-blue-300">
                                                        {new Date(activity.start_date).toLocaleDateString('en-US', { month: 'short' })}
                                                    </div>
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
                                                        {!activity.all_day && (
                                                            <span>⏰ {new Date(activity.start_date).toLocaleTimeString('en-US', { 
                                                                hour: '2-digit', 
                                                                minute: '2-digit' 
                                                            })}</span>
                                                        )}
