...
export default function EditRecipient({ recipient }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            { title: 'Edit', href: `/sas/admin/scholarship-recipients/${recipient.id}/edit` },
        ]}>
            ...
            <div className="flex-1 space-y-6 p-4 md:space-y-8 md:p-6 lg:p-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Edit Scholarship Recipient</h1>
                        <p className="text-sm text-muted-foreground">{recipient.student.name} - {recipient.scholarship.scholarship_name}</p>
                    </div>
                </div>

                <Card>
                    <CardHeader><CardTitle>Recipient Information</CardTitle><CardDescription>Update recipient details and status</CardDescription></CardHeader>
                    <CardContent>
                        <form onSubmit={submit} className="space-y-6">
                            <div className="grid gap-6 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    <Label>Amount</Label>
                                    <Input type="number" value={data.amount} onChange={e => setData('amount', parseFloat(e.target.value))} />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={data.renewal_status} onValueChange={value => setData('renewal_status', value)}>
                                        <SelectTrigger><SelectValue /></SelectTrigger>
                                        <SelectContent>
                                    ...
                                    {errors.renewal_status && <p className="text-sm text-red-600">{errors.renewal_status}</p>}
                                </div>

                                <div className="space-y-2 flex items-center">
                                    <Switch id="requirements_complete" checked={data.requirements_complete} onCheckedChange={checked => setData('requirements_complete', checked)} />
                                    <Label htmlFor="requirements_complete" className="ml-2">Requirements Complete</Label>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-4">
                                ...
                                <Button type="submit" disabled={processing}>{processing ? 'Updating...' : 'Update Recipient'}</Button>
                            </div>
                        </form>
                    </CardContent>
                </Card>
