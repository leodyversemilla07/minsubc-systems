...
export default function IncidentsReport({ year, byType, bySeverity, monthly }: { year: string; byType: Record<string, number>; bySeverity: Record<string, number>; monthly: MonthlyRow[] }) {
    return (
        <AppLayout>
            ...
            <div className="space-y-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold"><AlertTriangle className="mr-2 inline h-6 w-6" />Incidents Report</h1>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <Card className="p-6">
                        <CardTitle className="mb-4 text-lg">By Severity</CardTitle>
                        {bySeverity && Object.keys(bySeverity).length ? (
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {Object.entries(bySeverity).map(([sev, count]) => (
                                        <TableRow key={sev}>
                                            <TableCell className="capitalize">{sev}</TableCell>
                                            <TableCell className="text-right font-bold">{count}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                    ...
                    <Card className="p-6">
                        <CardTitle className="mb-4 text-lg">Monthly Trends</CardTitle>
                        {monthly?.length ? (
                            <Table>
                                <TableHeader>
                                    <TableRow>
