...
export default function AcademicTermEdit({ term }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        academic_year: term.academic_year,
        semester: term.semester,
        enrollment_start: term.enrollment_start ? term.enrollment_start.slice(0, 10) : '',
        enrollment_end: term.enrollment_end ? term.enrollment_end.slice(0, 10) : '',
        classes_start: term.classes_start ? term.classes_start.slice(0, 10) : '',
        classes_end: term.classes_end ? term.classes_end.slice(0, 10) : '',
        status: term.status,
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admission.admin.terms.update', term.id));
    };

    return (
        <>
            <Head title={`Edit Term: ${term.academic_year}`} />
            ...
            <div className="space-y-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Edit Academic Term</h1>
                        <p className="text-muted-foreground">{term.academic_year} — {term.semester}</p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="max-w-3xl">
                    ...
                    <div className="mt-6 flex justify-end gap-3">
                        <Link href={route('admission.admin.terms.show', term.id)} className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent">
                            Cancel
                        </Link>
...
AcademicTermEdit.layout = (page: React.ReactNode) => <AppLayout children={page} />;